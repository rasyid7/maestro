...
class Orchestra(
    ...
    private suspend fun executeCommand(maestroCommand: MaestroCommand, config: MaestroConfig?): Boolean {
        ...
        return when (command) {
            ...
            is BrowserAlertCommand -> browserAlertCommand(command)
            else -> true
        }.also { mutating ->
            if (mutating) {
                timeMsOfLastInteraction = System.currentTimeMillis()
    ...
    private fun browserAlertCommand(command: BrowserAlertCommand): Boolean {
        maestro.setBrowserAlertAction(command.action)
        return false
    }
    ...
    private suspend fun launchAppCommand(command: LaunchAppCommand): Boolean {
        ...
        try {
            ...
            val message = if (command.timeout != null) {
                "Unable to launch app ${command.appId} within ${command.timeout}ms"
            } else {
                "Unable to launch app ${command.appId}"
            }
            throw MaestroException.UnableToLaunchApp(message, cause = e)
        }
